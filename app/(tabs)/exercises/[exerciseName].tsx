...
export function ExerciseDetailScreenLegacy() {
  const router = useRouter();
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { exerciseName } = useLocalSearchParams();
  const { workouts, settings, customExercises, updateCustomExercise } = useGym();
  ...
  const exerciseNameStr = typeof exerciseName === 'string' ? exerciseName : '';
  ...
  // Load failure set data for rep max estimation
  ...
  // Load volume history and failure data on mount
  useEffect(() => {
    loadVolumeHistory();
    loadFailureSetData();
  ...
  // Reload volume history and failure data when page comes into focus (e.g., after completing a workout)
  useFocusEffect(
    useCallback(() => {
      loadVolumeHistory();
      loadFailureSetData();
  ...
  const currentExercise = useMemo(() => {
    ...
    const predefined = PREDEFINED_EXERCISES_WITH_MUSCLES.find(ex => ex.name === exerciseNameStr);
    if (predefined) return { ...predefined, isCustom: false };
  ...
  const handleSaveEdit = async (exerciseData: { name: string; primaryMuscle: MuscleGroup; secondaryMuscles: MuscleGroup[]; type: ExerciseType; muscleContributions: Record<MuscleGroup, number> }) => {
    ...
    // Navigate back and then to the new exercise name
    router.back();
    setTimeout(() => {
      router.push({
        pathname: '/(tabs)/exercises/[exerciseName]',
                ...
                <InteractiveLineChart
                  ...
                  tooltipData={filteredData.chartData.map((d) => ({
                    ...
                    lines: (() => {
                      ...
                      // Only add a separate volume line when the chart itself is not already volume.
...
// Keep the tab route in sync with the hidden route implementation.
// This is the actual screen used by expo-router for this route.
export { default } from '../../_hidden/exercises/[exerciseName]';
